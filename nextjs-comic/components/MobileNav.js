        ...
        <nav className="fixed mt-8 h-full w-full">
          {headerNavLinks.map((link) => (
            <div key={link.title} className="px-12 py-4">
              <Link
                href={link.href}
                className="text-gray-primary text-2xl font-bold tracking-widest"
                onClick={onToggleNav}
              >
                {link.title}
              </Link>
          ...
          <div className="border-theme ml-12 w-4/5 border-t px-12 py-4 pl-0 ">
            <ThemeSwitch description="Switch Theme" />
          </div>
        </nav>
      </div>
    </div>
